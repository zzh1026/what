 ...
 * 其作用是 :
 */
public class ActiveTicketSubDialog extends BaseDialog implements View.OnClickListener, HttpLoader.ResponseListener {
    ...
    public ActiveTicketSubDialog(ActiveActivity mContext, ActiveListResponse.DataBean.TakemeoutBean takemeoutBean) {
    ...
    private void initData() {
        ...
        String itemInfoId = takemeoutBean.getId();
        if (itemInfoId.length() == 1) {
            mActiveDialogTicketNum.setText("00" + itemInfoId);
        } else if (itemInfoId.length() == 2) {
            mActiveDialogTicketNum.setText("0" + itemInfoId);
        } else {
            mActiveDialogTicketNum.setText(itemInfoId);
        }
    }
    ...
    public void onGetResponseSuccess(int requestCode, RBResponse response) {
        if (requestCode == ConstantsWhatNSM.REQUEST_CODE_ACTIVE_TACK_JOIN
            ...
            if (activeJoinerTrideResponse.getCode() == 1) {
                ...
                ActiveJoinPayActivity.startActivePayForResult(mContext,
                        mActivePayTrideBean, ActiveActivity.JOIN_PAY_REQUEST_CODE);
                dismiss();
            } else {
